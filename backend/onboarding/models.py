...
class OnboardingForm(models.Model):
    ...
    # ── Meta ──────────────────────────────────────────────────
    pipedrive_deal_id = models.CharField(
        max_length=50, null=True, blank=True, unique=True
    )
    pipedrive_deal_name = models.CharField(max_length=255)
    assessor = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.PROTECT, related_name='onboardings'
    )
    ...
    modelo_venda = models.CharField(
        max_length=100, blank=True, default='', choices=ModeloVenda.choices
    )
    ...
    bonus_encontros = models.JSONField(
        default=list
    )  # [{label, date, time, responsible}, ...]
    ...
    fonte_conteudo_outro = models.CharField(max_length=255, blank=True, default='')
    como_descobriu_outro = models.TextField(blank=True, default='')
    decisivo_prospeccao_outro = models.TextField(blank=True, default='')
    experiencia_reuniao_outro = models.TextField(blank=True, default='')
    indicador_sucesso_outro = models.TextField(blank=True, default='')
...
class MaterialShare(models.Model):
    material = models.OneToOneField(
        GeneratedMaterial, on_delete=models.CASCADE, related_name='share'
    )
...
class OnboardingRuleAck(models.Model):
    rule = models.ForeignKey(
        OnboardingRule, on_delete=models.CASCADE, related_name='acks'
    )
    onboarding = models.ForeignKey(
        OnboardingForm, on_delete=models.CASCADE, related_name='rule_acks'
    )
