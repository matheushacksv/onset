...
<template>
  <div class="min-h-full p-6 md:p-10 max-w-xl mx-auto">
    ...
    <button
      class="text-sm text-white/50 hover:text-white/80 transition-colors mb-6 flex items-center gap-1.5"
      @click="router.back()"
    >
    ...
    <div class="mb-8">
      <h1 class="text-2xl font-semibold text-white">Novo funcionário</h1>
      <p class="text-sm text-white/40 mt-1">Crie uma conta de acesso à plataforma.</p>
    </div>

    <form
      class="bg-white/5 backdrop-blur-xl ring-1 ring-white/10 rounded-2xl p-6 space-y-5"
      @submit.prevent="submit"
    >

      <div>
        <label class="block text-xs font-medium text-white/60 uppercase tracking-widest mb-2">
          Nome
        </label>
        <input
          v-model="form.name"
      ...
      <div ref="roleMenuRef" class="relative">
        ...
        <button
          type="button"
          class="w-full bg-white/5 ring-1 ring-white/10 rounded-xl px-4 py-2.5 text-sm text-left flex items-center justify-between hover:ring-white/20 transition-shadow"
          :class="form.role.length ? 'text-white' : 'text-white/40'"
          @click="roleMenuOpen = !roleMenuOpen"
        >
          <span class="truncate">{{ roleLabel }}</span>
          <svg class="w-4 h-4 text-white/40 shrink-0 transition-transform" :class="{ 'rotate-180': roleMenuOpen }" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div
          v-if="roleMenuOpen"
          class="absolute z-10 left-0 right-0 mt-2 bg-neutral-900 ring-1 ring-white/10 rounded-xl overflow-hidden shadow-xl"
        >
          <div v-if="!availableRoles.length" class="px-4 py-3 text-sm text-white/40">
          ...
          <button
            ...
            :key="role"
            type="button"
            class="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-white hover:bg-white/5 transition-colors text-left"
            @click="toggleRole(role)"
          >
            <span
              class="w-4 h-4 rounded border ring-1 flex items-center justify-center shrink-0 transition-colors"
              :class="form.role.includes(role) ? 'bg-white border-white ring-white' : 'border-white/20 ring-white/10'"
            >
              <svg v-if="form.role.includes(role)" class="w-3 h-3 text-neutral-900" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3">
        ...
        <div v-if="form.role.length" class="flex flex-wrap gap-1.5 mt-2">
          <span
            ...
            :key="role"
            class="text-xs text-white/70 bg-white/5 ring-1 ring-white/10 px-2.5 py-1 rounded-full flex items-center gap-1.5"
          >
            {{ role }}
            <button
              type="button"
      ...
      <div class="flex justify-end gap-3 pt-2">
        ...
        <button
          type="submit"
          :disabled="!canSubmit"
          class="bg-white text-neutral-900 text-sm font-medium px-5 py-2.5 rounded-full hover:bg-white/90 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
        >
